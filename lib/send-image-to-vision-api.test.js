...
test('sendImageToVisionApi gets result', t=> {
  ...
  yielded = subject.next({size:1000});
  t.is(yielded.value.fn,fileDimensions);
  t.deepEqual(yielded.value.args, [filePath]);

  yielded = subject.next({width:11,height:10});
  t.is(yielded.value.fn,cloudinaryResizeImage);
  t.deepEqual(yielded.value.args, [fileExt,'base-64-contents',1000,11,10]);
